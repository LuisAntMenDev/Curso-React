...
export const TasksApp = () => {
    ...
    const [tasks, setTasks] = useReducer(tasksReducer, getTasksInitialState());
    const [inputValue, setInputValue] = useState('');

    const { todos, length: totalCount, completed: completedCount } = tasks;

    const addTodo = () => {
        setTasks({ type: 'ADD_TODO', payload: inputValue });
        setInputValue('');
    };

    const toggleTodo = (id: number) => {
        setTasks({ type: 'TOGGLE_TODO', payload: id });
    };

    const deleteTodo = (id: number) => {
        setTasks({ type: 'DELETE_TODO', payload: id });
    };
    ...
    useEffect(() => {
        localStorage.setItem('tasks-state', JSON.stringify(tasks));
    }, [tasks])
